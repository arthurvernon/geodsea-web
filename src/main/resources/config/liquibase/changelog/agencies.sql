...
CREATE TABLE boat.T_LICENSOR (
  ...
  ZONE                GEOMETRY (MULTIPOLYGON, 4326)
);
...
COMMENT ON COLUMN BOAT.T_CLUB.REPORT_RATE IS 'The default rate in seconds at which to request location updates. Otherwise event value is used.';
...
COMMENT ON COLUMN BOAT.T_RESCUE.REPORT_RATE IS 'The rate in seconds at which to request location updates from vessels within the zone.';
